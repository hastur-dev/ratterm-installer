#!/usr/bin/env bash
# Uninstall script for Emacs on Linux
...
uninstall_emacs() {
    ...
    log_info "Uninstalling Emacs using ${package_manager}..."
    ...
    while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y emacs emacs-common 2>&1; then
                    log_success "Emacs uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
                if sudo dnf remove -y emacs 2>&1; then
                    log_success "Emacs uninstalled successfully"
                    return 0
                fi
                ;;
            yum)
                if sudo yum remove -y emacs 2>&1; then
                    log_success "Emacs uninstalled successfully"
                    return 0
                fi
                ;;
            pacman)
                if sudo pacman -R --noconfirm emacs 2>&1; then
                    log_success "Emacs uninstalled successfully"
                    return 0
                fi
                ;;
...
verify_uninstall() {
    ...
    if command -v emacs &> /dev/null; then
        log_error "Emacs is still installed"
        return 1
    fi
...
# Main entry point
main() {
    log_info "Starting Emacs uninstallation on Linux..."
    ...
    uninstall_emacs "$package_manager"
    verify_uninstall
