...
readonly EMACS_EXPECTED_PATHS=("/usr/local/bin/emacs" "/opt/homebrew/bin/emacs" "/usr/bin/emacs")
...
# Main entry point
main() {
    log_info "Running Emacs verification on macOS..."
    ...
    if [ -z "$emacs_path" ]; then
        log_error "Emacs not found - please run install-macos.sh first"
        exit 1
    fi
