...
function Main {
    ...
    if (-not $installed) {
        ...
        $zip = "$env:TEMP\curlie.zip"
        Invoke-WebRequest -Uri $asset.browser_download_url -OutFile $zip
        Expand-Archive -Path $zip -DestinationPath "$env:TEMP\curlie" -Force
        $exe = Get-ChildItem -Path "$env:TEMP\curlie" -Recurse -Filter "curlie.exe" | Select-Object -First 1
        Copy-Item $exe.FullName -Destination "C:\Windows\System32\curlie.exe" -Force
