#!/usr/bin/env bash
# Install script for JavaScript (Node.js) on macOS
...
install_nodejs() {
    log_info "Installing Node.js (JavaScript runtime) via Homebrew..."

    if brew list node &> /dev/null; then
        log_info "Node.js is already installed, upgrading..."
        local attempt=0
        while [ $attempt -lt $MAX_RETRY_ATTEMPTS ]; do
            ((attempt++)) || true
            if brew upgrade node 2>&1 || true; then
    ...
    log_error "Failed to install Node.js after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify installation
verify_installation() {
    log_info "Verifying JavaScript (Node.js) installation..."

    if ! command -v node &> /dev/null; then
        log_error "node command not found after installation"
        return 1
    fi
...
# Main entry point
main() {
    log_info "Starting JavaScript (Node.js) installation on macOS..."
