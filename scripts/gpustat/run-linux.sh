#!/usr/bin/env bash
# Run script for gpustat on Linux
...
run_gpustat() {
    ...
    if "$gpustat_path" 2>&1; then
        ...
        log_info "gpustat returned non-zero (may indicate no NVIDIA GPU)"
        return 0
    fi
}
...
# Main entry point
main() {
    log_info "Running gpustat verification on Linux..."

    local gpustat_path
    gpustat_path=$(find_gpustat)

    if [ -z "$gpustat_path" ]; then
        log_error "gpustat not found - please run install-linux.sh first"
        exit 1
    fi
