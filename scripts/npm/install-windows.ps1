# Install script for npm on Windows
...
function Install-NodeWithWinget {
    Write-LogInfo "Installing Node.js (includes npm) via winget..."
    $attempt = 0
...
function Install-NodeWithChocolatey {
    Write-LogInfo "Installing Node.js (includes npm) via Chocolatey..."
    $attempt = 0
    while ($attempt -lt $MAX_RETRY_ATTEMPTS) {
        $attempt++
        try {
...
# Refresh PATH to find newly installed node/npm
...
function Test-NpmInstallation {
    Write-LogInfo "Verifying npm installation..."
    Update-PathEnvironment

    $npmCommand = Get-Command npm -ErrorAction SilentlyContinue
    if (-not $npmCommand) {
        Write-LogError "npm command not found after installation"
        return $false
    }
    ...
    try {
        ...
        Write-LogError "Failed to verify npm: $_"
        return $false
    }
}
...
# Main entry point
function Main {
    Write-LogInfo "Starting npm installation on Windows..."
    ...
    if (-not $installed) {
        Write-LogError "Failed to install Node.js/npm with any available package manager"
        exit 1
    }

    if (-not (Test-NpmInstallation)) {
        Write-LogError "npm installation verification failed"
        exit 1
    }
