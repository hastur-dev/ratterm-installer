#!/usr/bin/env bash
# Uninstall script for npm on Linux
...
# Uninstall Node.js and npm with retry
uninstall_nodejs() {
    ...
    log_info "Uninstalling Node.js and npm using ${package_manager}..."
        ...
        case "$package_manager" in
            apt-get)
                if sudo apt-get remove -y nodejs npm 2>&1; then
                    log_success "Node.js and npm uninstalled successfully"
                    return 0
                fi
                ;;
            dnf)
    ...
    log_error "Failed to uninstall Node.js/npm after ${MAX_RETRY_ATTEMPTS} attempts"
    return 1
}

# Verify uninstallation
verify_uninstall() {
    log_info "Verifying npm uninstallation..."
    ...
    log_success "npm has been removed from the system"
    return 0
}

# Main entry point
main() {
    log_info "Starting npm uninstallation on Linux..."

    if ! is_npm_installed; then
        log_info "npm is not installed, nothing to uninstall"
        exit 0
    fi
